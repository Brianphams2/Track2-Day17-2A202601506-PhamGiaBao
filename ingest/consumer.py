...
def write_batch(con: duckdb.DuckDBPyConnection, batch: list[dict]) -> None:
    ...
    Sử dụng INSERT ... ON CONFLICT (event_id) DO UPDATE để đảm bảo tính idempotent
    khi một batch được phát lại (replay).
    """
    con.executemany(
        f"""
...
def consume(
    ...
    with LogConsumer(topic, offset_file) as consumer:
        ...
        while True:
            ...
            # Sắp xếp lại theo at-least-once + idempotent write:
            # 1. Ghi dữ liệu xuống DB trước
            # 2. Crash có thể xảy ra
            # 3. Commit offset sau khi ghi thành công
